...
class LimitCancel(bt.Strategy):
    ...
    params = (  # Параметры торговой системы
        ('limit_pct', 0.0002)  # Заявка на покупку на n% ниже цены закрытия
    )
    ...
    def next(self):  
        ...
        self.logger.info(f'{self.params.limit_pct}')      
        ...
        if not self.position:  # Если позиции нет
            ...
            limit_price = self.data.close[0] * (1 - 0.003 / 100)  # На n% ниже цены закрытия
            
            self.logger.info(f'{limit_price}, {self.p.limit_pct}')
